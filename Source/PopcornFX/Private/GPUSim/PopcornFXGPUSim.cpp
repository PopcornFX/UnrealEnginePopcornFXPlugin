...
SRenderContext::ERHIAPI		g_PopcornFXRHIAPI = SRenderContext::_Unsupported;

void	SetupPopcornFXRHIAPI(uint32 API)
{
	PK_ASSERT(API <= SRenderContext::_Unsupported);
	g_PopcornFXRHIAPI = static_cast<SRenderContext::ERHIAPI>(API);
}
...
bool	_IsGpuSupportedOnPlatform(const EShaderPlatform &platform)
{
	...
	if (isStaticShaderPlatform)
	{
		...
		return	(
				 ...
				 platformName == FName("UNKNOWN3") ||
#endif // (defined(PK_DURANGO) && defined(PK_UNKNOWN)) || (PK_COMPILE_GPU_XBOX_ONE != 0)
#if defined(PK_UNKNOWN1) || (PK_COMPILE_GPU_UNKNOWN1 != 0)
				 platformName == FName("UNKNOWN1_SM6") ||
#endif // defined(PK_UNKNOWN1) || (PK_COMPILE_GPU_UNKNOWN1 != 0)
...
#if (PK_GPU_D3D11 == 1)
	if (g_PopcornFXRHIAPI == SRenderContext::D3D11)
		return My_RHICreateUnorderedAccessView_D3D11(VertexBufferRHI, Format);
#endif // (PK_GPU_D3D11 == 1)
#if (PK_GPU_D3D12 == 1)
	if (g_PopcornFXRHIAPI == SRenderContext::D3D12)
		return My_RHICreateUnorderedAccessView_D3D12(VertexBufferRHI, Format);
#endif // (PK_GPU_D3D12 == 1)
	PK_ASSERT_NOT_REACHED();
	return null;
}
