...
#define IMAGE_PLUGIN_BRUSH( RelativePath, ... ) FSlateImageBrush( FPopcornFXStyleEditor::InContent(RelativePath, ".png"), __VA_ARGS__ )

//----------------------------------------------------------------------------

TSharedPtr<FSlateStyleSet>	FPopcornFXStyleEditor::m_StyleSet;

//----------------------------------------------------------------------------

FString	FPopcornFXStyleEditor::InContent(const FString& RelativePath, const ANSICHAR* Extension)
{
	...
	return (pluginPopcornFX->GetContentDir() / RelativePath) + Extension;
}

//----------------------------------------------------------------------------

void	FPopcornFXStyleEditor::Initialize()
{
	if (!m_StyleSet.IsValid())
	{
		m_StyleSet = MakeShareable(new FSlateStyleSet(TEXT("PopcornFXStyleEditor")));
...
#define ATTRIBSAMPLER_STYLE(__name, __iconName, __imagePath) \
		...
		ATTRIBSAMPLER_STYLE("PopcornFXAttributeSamplerGrid", "AttributeSampler_Grid", "SlateBrushes/AttributeSampler_Grid");
...
void	FPopcornFXStyleEditor::Shutdown()
{
	if (m_StyleSet.IsValid())
	{
